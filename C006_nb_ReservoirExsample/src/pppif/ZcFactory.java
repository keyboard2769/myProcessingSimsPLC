...
public class ZcFactory {
  ...
  static class ZcDelayor extends ZcTimer{
    private int cmJudgeLine;
    public ZcDelayor(int pxOnDelay, int pxOffDelay){
      super(3);
      ccSetDelay(pxOnDelay, pxOffDelay);
    }//+++
    final void ccSetDelay(int pxOnDelay, int pxOffDelay){
      cmModel.ccSetRange(0, pxOnDelay+pxOffDelay);
      cmJudgeLine=pxOnDelay;
    }//+++
    @Override void ccAct(boolean pxAct){
      if(pxAct){cmModel.ccShift( 1);}
      else{cmModel.ccShift(-1);}
    }//+++
    @Override boolean ccIsUP(){return cmModel.ccGetCurrent()>cmJudgeLine;}//+++
  }//***
