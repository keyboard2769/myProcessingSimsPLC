...
public class ScFactory {
  ...
  static final JPanel ccMyFlowPannel(int pxGap, boolean pxAlignRight){
  ...
  //[PLAN]::ccMyGripPanel(int pxR, int pxC)
  ...
  static String ccGetStingFromInputBox(String pxBrief, String pxDefault){
    if(ccCheckEDT()){
      ...
      if(VcConst.ccIsValidOrNot(lpRes)){return lpRes;}
      else{return "<nc/>";}
    }
    return "<oedt/>";
  }//+++
  ...
  static class ScToolBarWindow extends JWindow {
    ...
    void ccInit(String pxTitle, ActionListener pxListener){
      ...
      //-- public
      cmFlow=new JPanel(new FlowLayout(FlowLayout.LEADING));
    ...
    void ccAdd(JComponent pxComponent){cmFlow.add(pxComponent);}//+++
